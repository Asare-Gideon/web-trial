...
const Page = () => {
  ...
  const { handler: handleAddCourseCaller, loading } = useAsyncCaller();
  ...
  async function onSubmit(values: z.infer<typeof addNewCourseSchema>) {
    if (user && thumbnails.length > 0) {
      ...
      try {
        await handleAddCourseCaller(async () => {
          // Upload thumbnails
          ...
          await addDoc(collection(FIREBASE_DB, "courses"), {
      ...
      toast({
        variant: "destructive",
        title: "Please add at least one thumbnail image",
      });
    }
  }

  const sendrq = async () => {
    const rawResponse = await fetch("/api/publish-scheduled-posts", {
      method: "POST",
      headers: {
        ...
        <div className="flex items-center justify-between mb-6 border-b pb-4">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={() => cleanForm()}>
              <RiDeleteBin5Line className="mr-2 h-4 w-4" /> Clear Form
            </Button>
          </div>
        </div>
