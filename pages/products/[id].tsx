...
const Product: React.FC<Props> = () => {
  ...
  const [value, productLoading, error] = useDocument(
    id ? doc(FIREBASE_DB, "products", id?.toString()) : null,
    {
    ...
    usePagination<ProductType>(
      productCategory
        ? query(
            PRODUCT_REF,
            where("visible", "==", true),
            where("productData.category", "==", productCategory)
          )
  ...
  useEffect(() => {
    setProduct({
      ...
      name: productData?.productData.name as any,
      price: productData?.productData.price as any,
      category: productData?.productData.category,
      date: productData?.date,
      description: productData?.productData.description,
      img1: productData?.productData.images[0],
      img2: productData?.productData.images[1],
      qty: productData?.productData.quantity,
    });
    setNewItem({
      id: productData?.id as any,
      name: productData?.productData.name as any,
    ...
    setProductCategory(productData?.productData.category);
  }, [productData]);
